...
class Test_network:
    ...
    def test_new_entity_login(self, setup):
        ...
        time.sleep(5)
        self.np.setstorename("KJaikisan KStore")
        self.np.clickstoretype()
        self.np.selectstoretype()
        self.np.clickstorestatus()
        time.sleep(3)
        self.np.selectStorestatusActive()
        time.sleep(3)
        self.np.clicksubmitStore()
        time.sleep(2)
        print(self.np.textstorename())
        act_title = self.np.textstorename()
        exp_title = "This Store already exists."
        if act_title == exp_title:
            self.logger.info("**** passed ****")
            self.np.setstorename("First Jaikisan")
            time.sleep(2)
        else:
            self.logger.info("**** failed ****")
            self.np.setstorename("Jaikisan Store")
            time.sleep(3)
        self.np.clicksubmitStore()
        time.sleep(5)
